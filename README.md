...
```
ggrep -r started transaction_data_daily_event_log_20190129.dat
```
...
```
ggrep -r started -c transaction_data_daily_event_log_20190129.dat
```


Use grep to find all instances where the upload was successful. 
```
ggrep -r completed  transaction_data_daily_event_log_20190129.dat
```
...
```
ggrep -r complete  transaction_data_daily_event_log_20190129.dat | wc -l
```
...
```
 ggrep -r -nP failure  transaction_data_daily_event_log_20190129.dat
```

Upon review, we would like to only view failures with error code SYSOFFLINE or WEAKSIGNAL.

```
 ggrep -r -nP SYSOFFLINE\|WEAKSIGNAL  transaction_data_daily_event_log_20190129.dat
```
...
```
grep - r ^[0-9]+	[A-Z][a-z\s\W]+	'Softaware|Developer'	[A-Z][a-z\s\W]+ [A-Z][a-z\s\W]+$ candidates_1.txt
```